...
| 구분     | 책임                              | 비고                                                                                                                                                   | 예시 (에러 메시지)                                                                                  |
|:-------|:--------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------|:---------------------------------------------------------------------------------------------|
| **입력**     | 방문 날짜를 입력받는다<br> 입력값을 검증한다      | 1~31 사이의 숫자만 입력 가능하다<br/> 유효하지 않은 입력시  에러 메시지를 출력한다                                                                                                  | "31일", "a", "!#$" <br>  ([ERROR] 유효하지 않은 날짜입니다. 다시 입력해 주세요.)                                 |
| **입력**     | 주문 메뉴와 개수를 입력받는다 <br> 입력값을 검증한다 | 에러 메시지를 출력하는 경우<br/>- 메뉴판에 없는 메뉴를 입력하는 경우<br/>- 주문 개수의 경우 1 이상의 숫자 이외의 입력값을 입력하는 경우<br/>- 메뉴 형식이 예시와 다른 경우<br/>- 중복 메뉴를 입력한 경우 | "뿌링클-2", "양송이수프-0", "시저샐러드2개", "시저샐러드-1,시저샐러드-2" <br> ([ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요.)    |
...
| **출력** | 12월 이벤트 배지 출력한다                 | 혜택 내역이 없다면 "없음"을 출력한다     |
...
할인과 이벤트 부분에서도, '증정'의 위치가 매우 모호했다. 증정 자체는 이벤트성이 강하나 할인 혜택 내역에 함께 포함되어야 하고 증정을 위해서는 총 주문 금액 또한 알아야 하므로 이벤트보다는 할인에 포함시키는 것이 적절하다고 판단했다.  

그러나 '증정' 혜택이 총 주문 금액에서 차감되어서는 안되므로 총 지불금액을 반환할 때 이 부분을 주의해주어야 했다. 이에 대해서는 더 좋은 개선점이 없는지 조금 더 고민해봐야할 것 같다. 
